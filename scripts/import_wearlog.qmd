...
#collect files
files <- filefinder(c("wearlog"), continuous = TRUE, individual = TRUE)
files <- c(files, 
           filefinder(c("wearlog_lanyard"), continuous = TRUE, individual = TRUE)
)
#import files
data <- 
...
data2 <- 
  read_csv(files[7:39], show_col_types = FALSE) |> 
  drop_na(redcap_repeat_instance) |> 
  mutate(
         record_id = str_replace(record_id, "UCR", "UCR_")
         ) |> 
